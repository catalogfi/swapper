...
describe("--- ATOMIC SWAP - ETHEREUM ---", () => {
    ...
    describe("- Atomic Swap - Initiate -", () => {
        ...
        it("Alice should not be able to initiate a swap with amount greater than her allowance", async () => {
            ...
            expect(
                await usdc.allowance(alice.address, atomicSwap.address)
        ...
        it("Alice should not be able to initiate a swap with amount greater than her balance", async () => {
            await usdc
                .connect(owner)
                .transfer(alice.address, ethers.utils.parseUnits("100", 6n));
        ...
        it("Alice should be able to initiate a swap", async () => {
            ...
            orderId1 = ethers.utils.sha256(
                ethers.utils.defaultAbiCoder.encode(
                    ["bytes32", "address"],
                    [ethers.utils.sha256(secret1), alice.address]
                )
            );
            await expect(
                atomicSwap
        ...
        it("Alice should be able to initiate another swaps with different secret", async () => {
            await usdc
                .connect(owner)
                .transfer(alice.address, ethers.utils.parseUnits("500", 6n));
            expect(await usdc.balanceOf(alice.address)).to.equal(
                ethers.utils.parseUnits("500", 6n)
            );
            let currentBlock = await ethers.provider.getBlockNumber();
            orderId2 = ethers.utils.sha256(
                ethers.utils.defaultAbiCoder.encode(
                    ["bytes32", "address"],
                    [ethers.utils.sha256(secret2), alice.address]
                )
            );
            await expect(
                atomicSwap
            ...
            orderId3 = ethers.utils.sha256(
                ethers.utils.defaultAbiCoder.encode(
                    ["bytes32", "address"],
                    [ethers.utils.sha256(secret3), alice.address]
                )
            );
            await expect(
                atomicSwap
            ...
            orderId4 = ethers.utils.sha256(
                ethers.utils.defaultAbiCoder.encode(
                    ["bytes32", "address"],
                    [ethers.utils.sha256(secret4), alice.address]
                )
            );
            await expect(
                atomicSwap
    ...
    describe("- Atomic Swap - Redeem -", () => {
        it("Bob should not be able to redeem a swap with no initiator", async () => {
            await expect(
                atomicSwap.connect(bob).redeem(randomBytes(32), randomBytes(32))
        ...
        it("Bob should not be able to redeem a swap with invalid secret", async () => {
            await expect(
                atomicSwap.connect(bob).redeem(orderId1, randomBytes(32))
            ).to.be.revertedWith("AtomicSwap: invalid secret");
        });
        ...
        it("Bob should not be able to redeem a swap with the same secret", async () => {
            await expect(
                atomicSwap.connect(bob).redeem(orderId1, secret1)
        ...
        it("Bob should receive the correct amount even if Charlie redeems with valid secret", async () => {
            await expect(atomicSwap.connect(charlie).redeem(orderId2, secret2))
                ...
                .withArgs(
                    orderId2,
                    ethers.utils.sha256(secret2),
                    ethers.utils.hexlify(secret2)
                );

            expect(await usdc.balanceOf(bob.address)).to.equal(
                ethers.utils.parseUnits("200", 6n)
            );
            expect(await usdc.balanceOf(charlie.address)).to.equal(
                ethers.utils.parseUnits("0", 6n)
            );
        });
    });
    ...
    describe("- Atomic Swap - Refund -", () => {
        ...
        it("Alice should be able to refund a swap after the locktime", async () => {
            ...
            expect(await usdc.balanceOf(alice.address)).to.equal(
                ethers.utils.parseUnits("300", 6n)
            );
        });
